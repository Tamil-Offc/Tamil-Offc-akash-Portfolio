  ...
  return (
    <div className="bg-background min-h-screen">
      <section className="container-apple py-24 text-center sm:py-32">
        ...
        <p className="animate-fade-up delay-200 mx-auto mt-8 max-w-2xl text-balance text-xl text-muted-foreground">
          A selection of projects that demonstrate my technical expertise and eye for detail.
        </p>
      </section>

      <section className="container-apple grid gap-12 pb-24">
        {projects.map((p, i) => (
          <article key={p.title} className="group grid md:grid-cols-2 gap-12 items-center">
            <div className={`rounded-[1rem] aspect-video border border-border flex items-center justify-center overflow-hidden hover:scale-[1.02] transition-all duration-500 shadow-xl ${i % 2 === 1 ? 'md:order-2' : ''}`}>
            ...
            <div className="flex flex-col gap-6">
              <div>
                <div className="flex flex-wrap gap-2 mb-4">
                  {p.stack.map(s => (
                    <span key={s} className="px-3 py-1 rounded-full bg-secondary text-primary text-xs font-bold uppercase tracking-wider">{s}</span>
                  ))}
                </div>
                <h2 className="text-4xl font-bold mb-4 tracking-tight">{p.title}</h2>
                <p className="text-xl text-muted-foreground leading-relaxed">{p.description}</p>
              </div>

              <ul className="space-y-3">
                {p.points.map(pt => (
                  <li key={pt} className="flex gap-3 text-foreground/80">
              ...
              <div className="flex gap-4 mt-4">
                ...
                <a href={p.links.github} className="btn-pill bg-secondary text-foreground hover:bg-secondary/80 flex items-center gap-2 border border-border">
        ...
        <div className="mt-24 rounded-[3rem] border-2 border-dashed border-border bg-card p-16 text-center shadow-inner">
          <Sparkles className="mx-auto mb-6 text-primary w-12 h-12" />
          <h3 className="text-2xl font-bold mb-4">The next big thing is in progress.</h3>
          <p className="text-muted-foreground text-lg max-w-lg mx-auto">I'm always working on something new. Check back soon for more exciting projects!</p>
        </div>
      </section>
    </div>
  );
